...
public class TutorialB : MonoBehaviour {
	...
	void Update () {
		...
		case 3:
			if (InputManager.GetAxis ("Mouse X") != 0f || InputManager.GetAxis ("Mouse Y") != 0f) {
				timer = timer + Time.deltaTime;
				if (InputManager.GetAxis ("Vertical") != 0f || InputManager.GetAxis ("Horizontal") != 0f)
					timer = timer + Time.deltaTime;
			}
		...
		case 12:
			if (InputManager.GetButtonDown ("Jump") && player.GetComponent<Hero_Movement> ().isBouncing)
				counter = counter + 1;
		...
		case 13:
			if (counter == 0) {
				Instantiate (gun, player.transform.position, player.transform.rotation);
				counter = 1;
			}
	...
	void OnGUI () {
		...
		case 1:
			...
			GUI.Box (new Rect (10, 70, 200, 275),	"Bieeenvenido a la segunda parte!\r\n " +
			"Este tutorial estará más orientado a la práctica de tiro y manejo de \r\n " +
			"la cámara en dicha area.\r\n" +
			"... \r\n" +
			"Ingresa al area de tiro\r\n" +
			"\r\n" +
			"\r\n" +
			"\r\n" +
